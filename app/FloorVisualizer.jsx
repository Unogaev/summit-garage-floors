...
const FINISHES = [
  {
    id: "midnight",
    name: "Midnight Stone",
    sub: "Black · Charcoal · Silver flake",
    badge: "Signature",
    swatchFrom: "#1C1C1C",
    swatchTo: "#3A3A3A",
    minP: 7, maxP: 12,
    ...
    layers: [
      { c: "#333", t: "Diamond grind + moisture barrier" },
      { c: "#555", t: "Dark polyurea base coat" },
      { c: "#888", t: "Black & charcoal flake broadcast" },
      { c: "#222", t: "UV-stable polyaspartic topcoat" },
    ],
    baseColor: [28, 28, 28],
    flakeColors: [
      [60,60,60],[90,90,90],[130,130,130],
      [180,180,180],[220,220,220],[40,40,40],
  ...
  },
  {
    ...
    layers: [
      { c: "#666", t: "Diamond grind + prep" },
      { c: "#888", t: "Gray polyurea base coat" },
      { c: "#AAA", t: "Silver & white flake broadcast" },
      { c: "#777", t: "Polyaspartic topcoat" },
    ],
    baseColor: [110, 110, 110],
    flakeColors: [
      [200,200,200],[240,240,240],[160,160,160],
      [255,255,255],[130,130,130],[180,180,180],
      [220,220,220],[145,145,145],
    ],
  ...
  },
  {
    ...
    layers: [
      { c: "#AAA", t: "Surface prep + primer" },
      { c: "#CCC", t: "White polyurea base coat" },
      { c: "#EEE", t: "White & silver flake system" },
      { c: "#DDD", t: "Clear polyaspartic topcoat" },
    ],
    baseColor: [220, 220, 218],
    flakeColors: [
      [255,255,255],[240,240,240],[200,200,200],
      [225,225,225],[210,210,210],[255,255,255],
      [190,190,190],[245,245,245],
    ],
  ...
  },
  {
    ...
    layers: [
      { c: "#8B7355", t: "Surface prep + repair" },
      { c: "#A0855A", t: "Tan polyurea base coat" },
      { c: "#C8AA88", t: "Desert blend flake broadcast" },
      { c: "#9A7B55", t: "Polyaspartic topcoat" },
    ],
    baseColor: [168, 140, 105],
    flakeColors: [
      [200,175,140],[220,195,160],[180,155,120],
      [240,215,180],[160,135,100],[210,185,150],
      [195,170,135],[230,205,170],
    ],
    pattern: "fullflake", gloss: 0.35,
  },
  {
    ...
    sub: "Deep Blue · Silver shimmer",
    badge: "Bold",
    swatchFrom: "#1A3A5C",
    swatchTo: "#2A5A8C",
    minP: 9, maxP: 14,
    life: "10–15 yrs", cure: "24–48 hrs",
    desc: "Deep cobalt metallic epoxy with silver shimmer. A bold statement for the serious car enthusiast.",
    layers: [
      { c: "#1A2A3A", t: "Surface prep + primer" },
      { c: "#1A3A6A", t: "Cobalt epoxy base" },
      { c: "#4A7AAA", t: "Metallic pigment blend" },
      { c: "#2A4A6A", t: "Urethane topcoat" },
    ],
    baseColor: [25, 55, 100],
    flakeColors: [],
    pattern: "metallic", gloss: 0.6,
    metallicSheen: [80, 140, 220],
  },
  {
    id: "gold",
    name: "Gold Metallic",
    sub: "Bronze · Gold · Copper shimmer",
    badge: "Premium",
    swatchFrom: "#7A5A10",
    swatchTo: "#C89A20",
    minP: 9, maxP: 15,
    ...
    layers: [
      ...
      { c: "#8B6914", t: "Epoxy base layer" },
      { c: "#C8A020", t: "Gold metallic pigment blend" },
      { c: "#5F5033", t: "Urethane protective topcoat" },
    ],
    baseColor: [140, 100, 20],
    flakeColors: [],
    pattern: "metallic", gloss: 0.65,
    metallicSheen: [240, 190, 60],
  },
];

function drawEpoxyFloor(ctx, polygon, finish, alpha) {
  ...
  if (finish.pattern === "fullflake") {
    ...
    const maxFlakes = Math.min(Math.floor((W*H)/18), 8000);
    for (let i = 0; i < maxFlakes; i++) {
      const [fr,fg,fb] = finish.flakeColors[Math.floor(rand(i,0)*finish.flakeColors.length)];
      const fx = minX+rand(i,1)*W, fy = minY+rand(i,2)*H;
      const fsize = 2.5+rand(i,3)*5, angle = rand(i,4)*Math.PI, aspect = 0.3+rand(i,5)*0.5;
      ...
      ctx.fill();
      if (rand(i,10)>0.6) { ctx.strokeStyle=`rgba(255,255,255,${0.15+rand(i,11)*0.25})`; ctx.lineWidth=0.5; ctx.stroke(); }
      ctx.restore();
    }
    const gg = ctx.createLinearGradient(minX,minY,minX+W*0.7,minY+H*0.3);
    ...
    ctx.fillStyle=gg; ctx.fillRect(minX,minY,W,H);
  } else {
    ...
    ctx.fillStyle=g; ctx.fillRect(minX,minY,W,H);
    for (let i = 0; i < 14; i++) {
      const sd = finish.id.charCodeAt(0)*(i+1);
      const rx=minX+((sd*7919)%1000)/1000*W, ry=minY+((sd*6271)%1000)/1000*H, rr=(20+((sd*3571)%100))*(W/400);
      ...
      ctx.fillStyle=rg; ctx.fillRect(minX,minY,W,H);
    }
    const sw=ctx.createLinearGradient(minX,minY,minX+W*0.5,maxY);
    ...
    ctx.fillStyle=sw; ctx.fillRect(minX,minY,W,H);
  }
  ctx.restore();
  ctx.save();
  ctx.beginPath();
  ...
  for (let i=1;i<polygon.length;i++) ctx.lineTo(polygon[i].x,polygon[i].y);
  ctx.closePath();
  ctx.strokeStyle="rgba(200,169,110,0.7)"; ctx.lineWidth=2; ctx.stroke();
  ctx.restore();
}
...
export default function FloorVisualizer() {
  ...
  const startLive = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (poly.length >= 3) {
        ...
        const sx = ov.width/(rect.width||1), sy = ov.height/(rect.height||1);
        const scaled = poly.map(p => ({ x: p.x*sx, y: p.y*sy }));
        const f = FINISHES.find(x => x.id===selRef.current);
        drawEpoxyFloor(ctx, scaled, f, (opRef.current/100)*0.9);
      }
    }, 100);
  }, []);

  useEffect(() => {
    if (phase==="drawing") startLive();
    return () => { if (timerRef.current) clearInterval(timerRef.current); };
  }, [phase, startLive]);

  useEffect(() => {
    if (phase==="result" && snapData && polygon.length>=3) applyToSnap(snapData, polygon, finish, opacity);
  }, [sel, opacity]);
  ...
  const startCam = async () => {
    try {
      ...
      if (vidRef.current) { vidRef.current.srcObject=stream; vidRef.current.onloadedmetadata=()=>{ vidRef.current.play(); setPhase("drawing"); }; }
    } catch { alert("Camera access denied — allow in browser settings."); }
  };

  const getPos = (e, el) => { const rect=el.getBoundingClientRect(); const src=e.touches?e.touches[0]:e; return { x:src.clientX-rect.left, y:src.clientY-rect.top }; };

  const handleDrawStart = (e) => {
    e.preventDefault(); if (phase!=="drawing") return;
    const dc=drawCanvasRef.current; if (!dc) return;
    dc.width=dc.offsetWidth; dc.height=dc.offsetHeight;
    setIsDrawing(true); setPolygon([getPos(e,dc)]);
  };

  const handleDrawMove = (e) => {
    e.preventDefault(); if (!isDrawing||phase!=="drawing") return;
    const dc=drawCanvasRef.current; if (!dc) return;
    const pos=getPos(e,dc);
    setPolygon(prev => {
      const next=[...prev,pos];
      const ctx=dc.getContext("2d"); ctx.clearRect(0,0,dc.width,dc.height);
      ctx.beginPath(); ctx.moveTo(next[0].x,next[0].y); next.forEach(p=>ctx.lineTo(p.x,p.y));
      ctx.strokeStyle="rgba(200,169,110,0.8)"; ctx.lineWidth=2; ctx.setLineDash([5,3]); ctx.stroke();
      return next;
    });
  };

  const handleDrawEnd = (e) => { e.preventDefault(); setIsDrawing(false); };

  const takeSnap = () => {
    const vid=vidRef.current; const sc=snapRef.current; if (!vid||!sc) return;
    sc.width=vid.videoWidth||1280; sc.height=vid.videoHeight||720;
    sc.getContext("2d").drawImage(vid,0,0);
    const data=sc.toDataURL("image/jpeg",0.93); setSnapData(data);
    if (timerRef.current) clearInterval(timerRef.current);
    applyToSnap(data,polygon,finish,opacity); setPhase("result");
  };

  const applyToSnap = (data, poly, f, op) => {
    if (!poly||poly.length<3) return;
    const sc=snapRef.current; if (!sc) return;
    const vid=vidRef.current;
    const out=document.createElement("canvas"); out.width=sc.width; out.height=sc.height;
    const ctx=out.getContext("2d"); const img=new Image();
    img.onload=()=>{
      ctx.drawImage(img,0,0);
      const dw=vid?vid.offsetWidth:sc.width, dh=vid?vid.offsetHeight:sc.height;
      const scaledPoly=poly.map(p=>({ x:p.x*(sc.width/(dw||1)), y:p.y*(sc.height/(dh||1)) }));
      drawEpoxyFloor(ctx,scaledPoly,f,(op/100)*0.93);
      setResultSrc(out.toDataURL("image/jpeg",0.93));
    }; img.src=data;
  };

  const reset = () => {
    setPhase("drawing"); setPolygon([]); setResultSrc(null); setSnapData(null); polygonRef.current=[];
    const dc=drawCanvasRef.current; if (dc) dc.getContext("2d").clearRect(0,0,dc.width,dc.height);
    const ov=ovRef.current; if (ov) ov.getContext("2d").clearRect(0,0,ov.width,ov.height);
  ...
  const download = () => { if (!resultSrc) return; const a=document.createElement("a"); a.href=resultSrc; a.download=`summit-garage-${sel}.jpg`; a.click(); };
    ...
    <section className="bg-[#0A0A09] border-t border-white/[0.07]">
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-10">
        ...
        <p className="text-[#6B6860] text-sm leading-relaxed max-w-lg">
          Point your phone at your garage floor, draw the area with your finger, and see any premium finish applied instantly.
        </p>
      </div>
      ...
      <div className="border-t border-b border-white/[0.07] grid grid-cols-2 md:grid-cols-4">
        {[
          ...
          {n:"03",title:"Draw the Floor",sub:"Trace the area with your finger"},
          {n:"04",title:"Capture & See",sub:"Realistic finish applied instantly"},
        ].map((s,i) => (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px]">
          <div className="border-r border-white/[0.07]">
            <div className="relative w-full aspect-[4/5] md:aspect-[16/10] bg-[#0D0D0C] overflow-hidden select-none">
              {phase==="idle" && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 text-center px-8">
                  ...
                  <div>
                    ...
                    <p className="text-[#6B6860] text-sm leading-relaxed max-w-[220px]">Enable camera · Draw floor area · See the finish</p>
                  </div>
                </div>
              )}
              <video ref={vidRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover"
              ...
              <canvas ref={drawCanvasRef} className="absolute inset-0 w-full h-full"
                style={{display:phase==="drawing"?"block":"none",touchAction:"none",cursor:"crosshair"}}
              ...
              {resultSrc&&phase==="result"&&<img src={resultSrc} alt="Floor preview" className="absolute inset-0 w-full h-full object-cover" />}
              {phase==="drawing"&&polygon.length===0&&(
                <div className="absolute top-4 left-0 right-0 flex justify-center pointer-events-none">
                  <div className="bg-black/85 border border-[#C8A96E]/40 px-4 py-2.5 rounded-sm">
                    <span className="text-[11px] tracking-[0.12em] uppercase text-[#C8A96E]">Hold and draw the floor area</span>
                  </div>
                </div>
              )}
              {phase==="drawing"&&(
                <div className="absolute top-4 left-4 flex items-center gap-2 bg-black/80 border border-white/10 px-3 py-1.5 rounded-sm pointer-events-none">
              ...
              {phase!=="idle"&&(
                <div className="absolute top-4 right-4 bg-black/80 border border-[#C8A96E]/25 px-3 py-1.5 rounded-sm pointer-events-none">
                  <span className="font-serif text-[#C8A96E] text-sm">{finish.name}</span>
                </div>
              )}
              {phase==="drawing"&&polygon.length>=3&&(
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-center gap-4 px-5 pb-6 pt-10"
                  ...
                  <button onClick={clearZone} className="text-[10px] tracking-[0.14em] uppercase text-white/50 border border-white/20 px-4 py-2.5 rounded-sm hover:text-white transition-all">Redraw</button>
            ...
            {phase==="drawing"&&(
              <div className="px-6 py-3 border-b border-white/[0.07]">
                <span className="text-[11px] text-[#6B6860]">{polygon.length<3?"Hold and drag to outline the floor area":"Floor outlined — tap the shutter to apply finish"}</span>
              </div>
            )}
            {phase==="result"&&(
              <div className="flex items-center gap-4 px-6 py-3.5 border-b border-white/[0.07]">
                ...
                <input type="range" min={40} max={98} value={opacity} step={1} onChange={e=>setOpacity(Number(e.target.value))} className="flex-1 accent-[#C8A96E]" />
              ...
              <div className="flex flex-col gap-2">
                {FINISHES.map(f=>(
                  <button key={f.id} onClick={()=>setSel(f.id)}
                    ...
                    <div className="w-8 h-8 rounded-sm flex-shrink-0 overflow-hidden relative" style={{background:`linear-gradient(135deg,${f.swatchFrom},${f.swatchTo})`}}>
                      {f.pattern==="fullflake"&&<div className="absolute inset-0" style={{backgroundImage:`radial-gradient(circle 1px at 20% 30%, rgba(255,255,255,0.8) 0%, transparent 50%), radial-gradient(circle 1.5px at 60% 70%, rgba(255,255,255,0.6) 0%, transparent 50%), radial-gradient(circle 1px at 80% 20%, rgba(255,255,255,0.9) 0%, transparent 50%), radial-gradient(circle 1px at 40% 80%, rgba(255,255,255,0.7) 0%, transparent 50%)`}} />}
                      {f.pattern==="metallic"&&<div className="absolute inset-0" style={{background:`linear-gradient(120deg, rgba(255,255,255,0.3) 0%, transparent 40%, rgba(255,255,255,0.2) 70%, transparent 100%)`}} />}
                    </div>
